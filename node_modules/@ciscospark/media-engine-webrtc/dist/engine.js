...
var WebRTCMediaEngine = (_dec = (0, _common.whileInFlight)('gumming'), _dec2 = (0, _lodashDecorators.debounce)(500), (_class = function () {
  ...
  function WebRTCMediaEngine() {
    ...
    _initDefineProp(this, 'localSlideShare', _descriptor3, this);
  ...
  (0, _createClass3.default)(WebRTCMediaEngine, [{
    ...
    value: function _prepareIceGatherer() {
      ...
      if (this.pc.iceGatheringState === 'new') {
        ...
        if (this.audioDirection !== 'inactive' || getTargetMediaDirection(this, 'audio') !== this.audioDirection && getTargetMediaDirection(this, 'audio') !== 'inactive') {
          var audio = sdp.media.find(function (m) {
            return m.type === 'audio';
          });
          if (audio) {
        ...
        if (this.videoDirection !== 'inactive' || getTargetMediaDirection(this, 'video') !== this.videoDirection && getTargetMediaDirection(this, 'video') !== 'inactive') {
          var video = sdp.media.find(function (m) {
            return m.type === 'video';
          });
          if (video) {
    ...
    value: function createOffer() {
      ...
      })).then((0, _webrtcHelpers.ensureH264)(wantsVideo)).then((0, _common.tap)(function (sdp) {
        _this3.offerSdp = sdp;
      }));
    }
    ...
    value: function _getUserMedia() {
      ...
      return new _promise2.default(function (resolve) {
        ...
        if (_this6.constraints.audio === true && _this6.pc.getSenders().find(function (s) {
          return s.track.kind === 'audio';
        })) {
          _this6.logger.info('already have a local audio track, removing constraint for a second one');
          (0, _deleteProperty2.default)(_this6.constraints, 'audio');
        }

        if (_this6.constraints.video === true && _this6.pc.getSenders().find(function (s) {
          return s.track.kind === 'video';
        })) {
          _this6.logger.info('already have a local video track, removing constraint for a second one');
          (0, _deleteProperty2.default)(_this6.constraints, 'video');
        }

        return WebRTCMediaEngine.getUserMedia(_this6.constraints);
      }).then(function (stream) {
        _this6.logger.info('got local media stream with ' + stream.getAudioTracks().length + ' audio tracks and ' + stream.getVideoTracks().length + ' video tracks');
        stream.getTracks().forEach(function (t) {
          return _this6.addOrReplaceTrack(t);
        });
        _this6.constraints = {};
      }).catch(function (err) {
    ...
    value: function pauseSendingMedia(kind) {
      ...
      senders.forEach(function (s) {
        _this7.logger.info('pausing ' + kind + ' sender');
    ...
    value: function unpauseSendingMedia(kind) {
      ...
      senders.forEach(function (s) {
        _this8.logger.info('unpausing ' + kind + ' sender');
    ...
    value: function pauseReceivingMedia(kind) {
      ...
      tracks.forEach(function (t) {
        _this9.logger.info('pausing remote ' + kind + ' track');
        t.enabled = false;
      });
    ...
    value: function unpauseReceivingMedia(kind) {
      ...
      tracks.forEach(function (t) {
        _this10.logger.info('unpausing remote ' + kind + ' track');
        t.enabled = true;
      });
  ...
  enumerable: true,
  initializer: null
}), _descriptor3 = _applyDecoratedDescriptor(_class.prototype, 'localSlideShare', [_commonEvented2.default], {
  enumerable: true,
  initializer: null
