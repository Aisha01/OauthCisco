...
var Locus = _sparkCore.SparkPlugin.extend({
   ...
   * @returns {string} one of USE_INCOMING, USE_CURRENT, or FETCH
   */
  _compareDelta: function _compareDelta(current, incoming) {
    var ret = this._compareSequence(current.sequence, incoming.sequence);
    if (ret !== LESS_THAN) {
   ...
   */
  _compareSequence: function _compareSequence(current, incoming) {
    if (!current) {
