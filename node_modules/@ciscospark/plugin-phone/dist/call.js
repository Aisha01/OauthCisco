...
var Call = _sparkCore.SparkPlugin.extend((_dec = (0, _common.whileInFlight)('locusJoinInFlight'), _dec2 = (0, _common.whileInFlight)('locusLeaveInFlight'), (_obj = {
  ...
  hangup: function hangup() {
    ...
    if (!this.locus) {
      return this._finish().then(function () {
        return _this7.logger.info('call: hang up complete, call never created');
      });
    }
  ...
  initialize: function initialize() {
    ...
    this.listenTo(this.media, 'negotiationneeded', (0, _debounce3.default)(function () {
      _this8.media.createOffer().then(function () {
        ...
        return _this8._pollForExpectedLocusAndSdp();
      }).then(function () {
    ...
    this.on('change:localMediaStream', function () {
      if (_this8.localMediaStream && _this8.localMediaStream !== _this8.media.localMediaStream) {
        ['audio', 'video'].forEach(function (kind) {
          // eslint-disable-next-line max-nested-callbacks
          var track = _this8.localMediaStream.getTracks().find(function (t) {
            return t.kind === kind;
          });
          if (track) {
            _this8.media.setMedia(kind, (0, _boolToStatus2.default)(track.enabled, _this8['receiving' + capitalize[kind]]), track);
          } else {
            _this8.media.setMedia(kind, (0, _boolToStatus2.default)(false, _this8['receiving' + capitalize[kind]]));
          }
        });
        _this8.localMediaStream = _this8.media.localMediaStream;
      }
    ...
    this.listenTo(this.media, 'internalTrackUpdate', function () {
      _this8.trigger('remoteMediaStream:change');
    });
    ...
    if (this.config.enableExperimentalGroupCallingSupport) {
      ...
      this.on = (0, _wrap3.default)(this.on, function (fn, eventName) {
        ...
        return (0, _apply2.default)(fn, _this8, [eventName].concat(rest));
      });
    } else {
  ...
  toggleFacingMode: function toggleFacingMode() {
    ...
    return new _promise2.default(function (resolve) {
      _this10.media.once('change:answerSdp', function () {
        _this10.facingMode = constraint.facingMode.ideal;
        resolve();
      });
    });
  },
  ...
  _changeReceivingMedia: function _changeReceivingMedia(kind, value) {
    var _this11 = this;
    ...
    return new _promise2.default(function (resolve) {
      ...
      if (!section || !section.direction.includes('recv')) {
        _this11.logger.info('_changeReceivingMedia: expecting to renegotiate, waiting for media to emit "answeraccepted"');
        _this11.media.once('answeraccepted', function () {
          return resolve(_this11._updateMuteToggles());
        });
      } else {
        _this11.logger.info('_changeReceivingMedia: expecting to renegotiate, waiting for call to emit "change:receiving' + capitalize[kind] + '"');
        _this11.once('change:receiving' + capitalize[kind], function () {
          return resolve();
        });
      }

      var newDirection = (0, _boolToStatus2.default)(_this11.media['sending' + capitalize[kind]], value);
      _this11.media.setMedia(kind, newDirection);
    });
  },
  _changeSendingMedia: function _changeSendingMedia(kind, value) {
    var _this12 = this;
    ...
    return new _promise2.default(function (resolve) {
      ...
      if (!section || !section.direction.includes('send')) {
        _this12.logger.info('_changeSendingMedia: expecting to renegotiate, waiting for media to emit "answeraccepted"');
        _this12.media.once('answeraccepted', function () {
          return resolve();
        });
      } else {
        _this12.logger.info('_changeSendingMedia: expecting to renegotiate, waiting for call to emit "change:sending' + capitalize[kind] + '"');
        _this12.once('change:sending' + capitalize[kind], function () {
          return resolve(_this12._updateMuteToggles());
        });
      }

      var newDirection = (0, _boolToStatus2.default)(value, _this12.media['receiving' + capitalize[kind]]);
      _this12.media.setMedia(kind, newDirection);
    });
  },
  ...
  _hangup: function _hangup() {
    ...
    return this.spark.internal.locus.leave(this.locus).catch(function (err) {
      _this13.logger.error('_hangup: locus leave error: ', err.stack || err.toString());
      return _promise2.default.reject(err);
    }).then((0, _common.tap)(function () {
      return _this13.logger.info('_hangup ' + _this13.locus.id + ': finished leaving via locus');
    })).then((0, _common.tap)(function () {
      return _this13.logger.info('_hangup ' + _this13.locus.id + ': setting locus');
    })).then(function (locus) {
      return _this13._setLocus(locus);
    }).then((0, _common.tap)(function () {
      return _this13.logger.info('_hangup ' + _this13.locus.id + ': finished setting locus');
    }))
  ...
  _finish: function _finish() {
    ...
    return new _promise2.default(function (resolve) {
      ...
      process.nextTick(function () {
        _this14.media.stop();
        _this14.stopListening(_this14.spark.internal.mercury);
        _this14.off();
        resolve();
      });
    });
  ...
  _join: function _join(locusMethodName, target) {
    ...
    return this.media.createOffer().then((0, _common.tap)(function () {
      return _this15.logger.info('created offer');
    })).then(function () {
      return _this15.spark.internal.locus[locusMethodName](target, {
        localSdp: _this15.media.offerSdp,
        correlationId: _this15.correlationId
      });
    }).then((0, _common.tap)(function () {
      return _this15.logger.info('sent offer to locus');
    })).then((0, _common.tap)(function () {
      return _this15.logger.info('setting locus');
    })).then(function (locus) {
      return _this15._setLocus(locus);
    }).then((0, _common.tap)(function () {
      return _this15.logger.info('successfully set locus');
    })).then(function () {
      var answer = JSON.parse(_this15.mediaConnection.remoteSdp).sdp;
      _this15.logger.info('accepting answer');
      _this15.logger.info('peer state', _this15.media.pc && _this15.media.pc.signalingState);
      if (!_this15.media.ended) {
        return _this15.media.acceptAnswer(answer).then(function () {
          return _this15.logger.info('answer accepted');
        }).catch(function (err) {
          _this15.logger.error('failed to accept answer', err);
          return _promise2.default.reject(err);
        });
      }
      _this15.logger.info('call: already ended, not accepting answer');
      return _promise2.default.resolve();
    });
  },
  ...
  _onLocusEvent: function _onLocusEvent(event) {
    ...
    var device = devices && (0, _find3.default)(devices, function (item) {
      return item.url === _this16.spark.internal.device.url;
    });
   ...
   */
  _pollForExpectedLocusAndSdp: function _pollForExpectedLocusAndSdp() {
    var _this17 = this;

    return new _promise2.default(function (resolve, reject) {
      var offer = (0, _sdpTransform.parse)(_this17.media.offerSdp);

      var audioOfferDirection = getMediaDirectionFromSDP('audio', offer);
      var videoOfferDirection = getMediaDirectionFromSDP('video', offer);
      ...
      var validate = function validate() {
        count += 1;
        try {
          _this17.logger.info('iteration ' + count + ': checking if current locus has the expected audio and video directions');
          if (isExpectedDirection(JSON.parse(_this17.mediaConnection.remoteSdp).sdp)) {
            _this17.logger.info('iteration ' + count + ': the current locus sdp has the expected audio and video directions');
            try {
              _this17._validateLocusMediaState(_this17.locus);
              _this17.logger.info('iteration ' + count + ': the current locus has the expected audio and video directions; syncing');
              resolve();
            } catch (err) {
              _this17.logger.info('iteration ' + count + ': the current locus does not have the expected audio and video directions; syncing');
            }
          } else {
            _this17.logger.info('iteration ' + count + ': the current locus sdp does not have the expected audio and video directions; syncing');
          }
          ...
          setTimeout(function () {
            return _this17.spark.internal.locus.sync(_this17.locus).then(function (locus) {
      ...
      function isExpectedDirection(sdp) {
        ...
        var audioAnswerDirection = getMediaDirectionFromSDP('audio', answer);
        var videoAnswerDirection = getMediaDirectionFromSDP('video', answer);

        var expectedAudioAnswerDirection = reverseMediaDirection(audioOfferDirection);
        var expectedVideoAnswerDirection = reverseMediaDirection(videoOfferDirection);
      ...
      case _internalPluginLocus.FETCH:
        if (recursing) {
          ...
          return this.spark.internal.locus.get(current).then(function (locus) {
            return _this18._setLocus(locus, true);
          });
        }
  ...
  _updateMedia: function _updateMedia(payload) {
    ...
    return this.spark.internal.locus.updateMedia(this.locus, payload).then(function (locus) {
      return _this19._setLocus(locus);
    });
  },
   ...
   * @returns {Promise}
   */
  _updateMuteToggles: function _updateMuteToggles() {
    var _this20 = this;
    ...
    return this._updateMedia({
      ...
      return _this20._waitForExpectedLocus();
    });
  },
  ...
  _waitForExpectedLocus: function _waitForExpectedLocus() {
    ...
    return new _promise2.default(function (resolve) {
      var listener = function listener() {
        try {
          _this21._validateLocusMediaState(_this21.locus);
          _this21.off('change:locus', listener);
          resolve();
        } catch (err) {
          _this21.logger.warn('locus: current locus not in expected state; waiting for next locus');
        }
      };
      _this21.on('change:locus', listener);
      listener();
    });
  },
  version: '1.23.2'
}, (_applyDecoratedDescriptor(_obj, 'acknowledge', [_common.oneFlight], (0, _getOwnPropertyDescriptor2.default)(_obj, 'acknowledge'), _obj), _applyDecoratedDescriptor(_obj, 'answer', [_common.oneFlight, _dec], (0, _getOwnPropertyDescriptor2.default)(_obj, 'answer'), _obj), _applyDecoratedDescriptor(_obj, 'dial', [_common.oneFlight], (0, _getOwnPropertyDescriptor2.default)(_obj, 'dial'), _obj), _applyDecoratedDescriptor(_obj, 'reject', [_common.oneFlight], (0, _getOwnPropertyDescriptor2.default)(_obj, 'reject'), _obj), _applyDecoratedDescriptor(_obj, '_hangup', [_common.oneFlight, _dec2], (0, _getOwnPropertyDescriptor2.default)(_obj, '_hangup'), _obj), _applyDecoratedDescriptor(_obj, '_join', [_common.oneFlight], (0, _getOwnPropertyDescriptor2.default)(_obj, '_join'), _obj), _applyDecoratedDescriptor(_obj, '_pollForExpectedLocusAndSdp', [_common.retry], (0, _getOwnPropertyDescriptor2.default)(_obj, '_pollForExpectedLocusAndSdp'), _obj), _applyDecoratedDescriptor(_obj, '_updateMuteToggles', [_common.oneFlight], (0, _getOwnPropertyDescriptor2.default)(_obj, '_updateMuteToggles'), _obj)), _obj)));
