...
var Credentials = _sparkPlugin2.default.extend((_dec = (0, _common.oneFlight)({ keyFactory: function keyFactory(scope) {
  ...
  invalidate: function invalidate() {
    ...
    tokens.forEach(function (token) {
      try {
        _this6.userTokens.remove(token);
      } catch (err) {
  ...
  refresh: function refresh() {
    ...
    if (this.config.jwtRefreshCallback) {
      return this.config.jwtRefreshCallback(this.spark).then(function (jwt) {
        return _this7.spark.authorization.requestAccessTokenFromJwt({ jwt: jwt });
      });
    }

    return supertoken.refresh().then(function (st) {
      // clear refresh timer
      if (_this7.refreshTimer) {
        clearTimeout(_this7.refreshTimer);
        _this7.unset('refreshTimer');
      }
      _this7.userTokens.reset();
      _this7.supertoken = st;
      return _promise2.default.all(tokens.map(function (token) {
        _this7.logger.info('credentials: revoking token for ' + token.scope);
        return _this7.downscope(token.scope)
        // eslint-disable-next-line max-nested-callbacks
        .then(function (t) {
          _this7.userTokens.add(t);
          token.revoke();
        });
      }));
    }).then(function () {
      _this7.scheduleRefresh(_this7.supertoken.expires);
    });
  },
